...
export default function ImprovedCarouselGrid({ items }: Props) {
  ...
  // including the itemName for each result
      ...
      <Carousel>
        <div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-3 gap-2">
          {allResults.map((result, idx) => (
            <ProductCard
              key={idx}
              itemName={result.itemName} // Passing the itemName here
...
export function FashionSearchChat() {
  ...
  const sendMessage = async (message: string, botResponse?: BotResponse) => {
    ...
    const result = await checkChatWithTalk(currentConversation, userLanguage);
