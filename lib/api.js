...
export function getFunilAnalytics(sellerId, filters = {}) {
  const queryParams = new URLSearchParams({ sellerId });
  ...
  console.log(`Requesting funnel analytics with params: ${queryParams.toString()}`);
  return request(`/getFunilAnalytics?${queryParams.toString()}`, 'GET')
    .then(data => {
